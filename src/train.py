...
class ProjectAgent:
    def __init__(self, num_actions, gamma=0.99, iterations= 500):
...
def rf_fqi(S, A, R, S2, D, iterations, nb_actions, gamma, disable_tqdm=False):
    ...
    for iter in tqdm(range(iterations), disable=disable_tqdm):
        ...
        Q = RandomForestRegressor(max_depth =5, min_samples_split=4)
        Q.fit(SA, value)
    return Q  
...
S, A, R, S2, D = collect_samples(env, horizon = 800)
